...
const create = (E_S_PK, E_NM, E_STR_DT, E_LIMIT, E_REG_U_PK) => {  // 시험 추가
  return new Promise(async (resolve, reject) => {
    try {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?);`;
      await promisePool.execute(examCreSql, [E_PK, E_S_PK, E_UG_PK, E_REG_U_PK, E_NM, E_STR_DT, E_LIMIT]);
      resolve({ 'status': 'success', 'msg': '시험 추가에 성공했습니다.' });
    } catch (err) {
      reject(err);
    }
  });
...
const list = (LIMIT) => {  // 시험 리스트 조회
  return new Promise(async (resolve, reject) => {
    try {
      const examsSql = `SELECT S.S_NM, E.E_NM, E.E_STR_DT, E.E_LIMIT, E.E_PK FROM ds2team.Exam AS E
      JOIN ds2team.Subject AS S ON S.S_PK = E.E_S_PK
